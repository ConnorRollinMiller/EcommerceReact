...
export const postNewReview = (shoeId, username, rating, reviewText) => {
   return dispatch => {
      if (!rating) {
         return dispatch(postNewReviewFailure('Must Give A Rating.'));
      }
      if (!reviewText) {
         return dispatch(postNewReviewFailure('Must Give A Review.'));
      }
      if (!shoeId) {
         return dispatch(postNewReviewFailure('No Shoe ID Provided.'));
      }
      if (!username) {
         return dispatch(
            postNewReviewFailure('Must Be Logged In To Leave Review.')
         );
      }
